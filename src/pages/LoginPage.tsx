...
export const LoginPage: React.FC = () => {
  ...
  const submitLogin = useCallback(
    ...
    [loginWithPin, selectedStaff, navigate]
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.target as HTMLElement)?.tagName === 'INPUT' && (e.target as HTMLElement)?.id === 'login-staff-search') {
        return;
      }
  ...
  }, [pin, handleDigitPress, handleBackspace, submitLogin]);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4 relative overflow-hidden selection:bg-orange-500">
      ...
      <div className="absolute top-6 left-6 z-10">
        <Link
          to="/"
          className="flex items-center space-x-2 px-4 py-2.5 rounded-2xl bg-slate-900/80 hover:bg-slate-800 border border-slate-800 text-xs font-bold text-slate-300 hover:text-white transition-all shadow-md backdrop-blur-md"
        >
      ...
      >
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Identify your account and authenticate with your 4-digit PIN
          </p>
        </div>

        {/* Currently logged in alert */}
        {currentStaff && (
          <div className="mb-4 p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-between text-xs text-emerald-300">
            <div className="flex items-center space-x-2">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span>
                Active Session: <strong>{currentStaff.name}</strong> ({currentStaff.role})
              </span>
            </div>
            <button
              onClick={logout}
